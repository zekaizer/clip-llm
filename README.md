...
- **Vision support** — images reach the model in Summarize / Explain / Transcribe on both providers: a copied image, image files, or the pictures inside a rich-text selection (browser, Word, Outlook, Notes), which arrive only as `<img>` references in the HTML flavor and are resolved by scheme (`data:`, Office's `file:` temp images, intranet `http(s)`). Markup images pass a meaningfulness filter (display and pixel size, aspect ratio, flat color, emoji/icon/avatar/logo hints, SVG) so decoration is never sent; a copied image or file is deliberate and always is. Every image is bounded before it goes out: a 50-megapixel decode budget checked from the header, flat margins trimmed then the long edge scaled to 1568px, JPEG instead of PNG when the PNG exceeds 1.5 MB, at most 4 images and 8 MiB per request, and `width×height/750` counted toward `token_budget`. Every request carries the clipboard text inside `<content>`; with images, a `<metadata>` block (attachment count and sizes, marked as the application's) leads so the model knows what was shrunk. The source badge tooltip lists the same
